  ...
  return (
    <div className='w-ful flex h-screen flex-col space-y-6'>
      ...
      <div className="bg-surface-100 border border-gray-200 text-lg font-medium text-gray-700 -mb-0 px-8 py-2">
        {loading ? (<Loader className="!w-[28px] !h-[28px]" />) : (showTable ? (`Date: ${formatDateToDDMMYYYY(startDate)} to ${formatDateToDDMMYYYY(endDate)}`) : ("Choose a start and end date to view attendance records."))}
      </div>
      {loading ? (<div className="flex justify-center h-full items-center"><Loader className="text-primary-100 w-[275px] h-[275px] border-25" /></div>) : (showTable ? (<div className="space-y-6 flex-1 overflow-auto pb-4 pt-6">
        {data.map((dept) => {
          return (
            <div className="px-8">
      ...
      </div>) : (<div className="flex justify-center h-full items-center"><ClockFading className="text-primary-100 w-[300px] h-[300px]" /></div>))}
    </div>
  );
};
