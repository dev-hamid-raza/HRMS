...
const Login = () => {
    ...
    const [errors, setErrors] = useState({usernameError: false, passwordError: false})
    ...
    const handleSubmit =async (e: React.MouseEvent<HTMLButtonElement>) => {
        ...
        if(password === '' || password === undefined) {
            ...
            try {
                await postData({password,username})
                toast.success("Login successfully")
                setIsAuthenticated(true)
            } catch (error) {
                const message = error as string
                if(message.includes('username')) setErrors((pervs) => ({...pervs, usernameError: true}))
                if(message.includes('password')) setErrors((pervs) => ({...pervs, passwordError: true}))
                toast.error(message)
            }
    }
    return (
